...
export class VerificacionReinyectorComponent implements OnInit {
  ...
  guardar(){
    ...
    this.verificarProduccionService.transCrearVerificarReinyector(this.verificacionProduccion).subscribe(data =>{
     ...
     this.verificacionProduccion.fecha_actualizacion = this.today
        ...
        this.loading = false;
        this.messageService.add({ severity: 'success', detail: 'Creación Exitosa' });
        this.obtenerTodo();

      } else {
        this.loading = false;
        this.messageService.add({ severity: 'info', detail: 'Error al Crear' });
  ...
  this.verificacionProduccion.Porcentaje_inc_dis= parseFloat( this.verificacionProduccion.PorcentajeMostrar);
  ...
  editarProduccion(verificacionProduccion: VerificacionProduccion, produccion: Produccion) {
    ...
    this.router.navigate(['/menu', { outlets: { sitp: ['EditarVerificacionReinyectorComponent'] } }]);
  }
